...
function template_backpath($file=""){
		...
		if($file!=""){
			...
			return "backend/".$CI->config->item("back_template")."/template/body";
		}
}
//assets backend
...
switch ($type) {
  case 'success':
  $html='<div class="alert alert-success auto-show">
  <span class="glyphicon glyphicon-ok"></span>&nbsp;&nbsp;&nbsp;'.$msj.'
</div>';
break;
case 'error':
$html='<div class="alert alert-danger auto-show">
<span class="glyphicon glyphicon-remove"></span>&nbsp;&nbsp;&nbsp;'.$msj.'
</div>';
break;
case 'info':
$html='<div class="alert alert-info auto-show">
<span class="glyphicon glyphicon-info-sign"></span>&nbsp;&nbsp;&nbsp;'.$msj.'
</div>';
break;
default:
...
function menu_get($unicode,$render=false){
    ...
    if($render && $menu!=false){
      ...
      foreach($menu as $i => $item):
          echo '<li><a href="'.base_url($item->url).'">'.$item->label.'</a></li>';
      endforeach;
     echo '</ul>';
    }
    return $menu;
...
function render_shorcodes($html){
  ...
  foreach($shortcodes as $i => $item):
    ...
    foreach($matches as $ii => $var):
      ...
      preg_match_all('/(?P<variable>\w+)=\"(?P<value>([a-zA-Z0-9])+([a-zA-Z0-9\._-])+)\"/',$var, $output_array);
      $variables=$output_array["variable"];
      $values=$output_array["value"];

      preg_match_all('/(?P<variable>\w+)=\"(?P<value>\d+)\"/',$var, $output_array);
...
/*function send_mail($mail,$subject,$html,$hiddens=""){
  ...
  function send_mail($mail,$subject,$html,$hiddens=null){
    ...
    $from_email="bhpay@hipotecario.com.sv";
    $from="Banco Hipotecario";
    $CI->load->library('PHPMailerCI',$config);
    $CI->phpmailerci->setFrom($from_email,$from);
    $CI->phpmailerci->AddReplyTo($from_email,$from);
    
    $message="";
    $r=$CI->phpmailerci->send_mail($mail,$subject,$html,$message);
    return $r;
  }
  ...
  function decryptToken($token){
    ...
    $minutes = (strtotime(date('Y-m-d H:i:s'))-strtotime($encrypt->timestamp))/60;
